...
What steps should you follow to achieve this in your API route? Have a look below:
    ...
    3. Insert the user input variables into a dictionary and append it to a DataFrame
...
def predict():
    ...
    for variable in variables:
        if variable in ("average_monthly_hours", "number_project"):
            ...
            dict_x[variable] = satisfaction_level_mean
    ...
    dict_x["hours_per_project"] = (
        dict_x["average_monthly_hours"] / dict_x["number_project"]
    )
    ...
    # Encode salary
    df.replace(salary_encoding, inplace=True)

    # Scale monthly hours
    df["average_monthly_hours"] = avg_monthly_hours_scaler.transform(
        df["average_monthly_hours"].values.reshape(1, -1)
    )
...
def prediction_transformation(
    prediction_set,
    avg_monthly_hours_scaler,
    ...
    prediction_set_copy = pd.concat(
        [prediction_set_copy, pd.get_dummies(prediction_set_copy["department"])], axis=1
    )
    prediction_set_copy.drop("department", axis=1, inplace=True)
    prediction_set_copy.replace(salary_encoding, inplace=True)
    prediction_set_copy["hours_per_project"] = (
        prediction_set_copy["average_monthly_hours"]
        / prediction_set_copy["number_project"]
    )
    prediction_set_copy["average_monthly_hours"] = avg_monthly_hours_scaler.transform(
    ...
    prediction_set_copy["last_evaluation"] = (
        prediction_set.last_evaluation.str.replace("%", "").astype(int) * 0.01
    )
...
def predict_all():
    ...
    try:
        df = pd.read_csv("HR_new_recruitment.csv", header=0, sep=",")
        X_predict = prediction_transformation(
            df,
            avg_monthly_hours_scaler,
            satisfaction_level_mean,
            salary_encoding,
...
def pred_post():
    # We wan to extracte a dictionary with all variables:values pairs
